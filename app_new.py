...
async def handle_upload(e):
    """處理文件上傳"""
    global current_file_path, pdf_pages, current_page
...
def show_ocr_result(content: str, original_filename: str):
    ...
    if not content:
        ui.notify("錯誤: 檢查檔案格式是否正確", type='negative')
        return
    
    # 確保結果容器存在
    if not hasattr(ui, 'ocr_result_container'):
        # 在 preview_container 下方創建結果容器
        with preview_container:
            ui.ocr_result_container = ui.column().classes('w-full mt-4')
    else:
        # 清空現有內容
        ui.ocr_result_container.clear()
...
if __name__ in ["__main__", "__mp_main__"]:
    ...
    create_ui()
    ui.run(title="Document Assistant", port=8080, reload=True, show=False)
